...
class Classifier(object):
    ...
    def predict(self, features, file_name, face_coords, store_features):
        if self.clf:
            ...
            logging.error("Missing model for: " + self.hashed_username)
    ...
    def train(self, conn):
        ...
        functions.set_team_training(conn, self.hashed_username)
        ...
        X, Y, sw, results = self._get_training_data(conn)
        ...
        for y in Y:
            if y > 0:
                new_Y.append(y)
        uni = np.bincount(new_Y)
        num = np.nonzero(uni)[0]
        cnt_uni = list(zip(num, uni[num]))

        print("fitted %d features from %d classes" % (len(X), len(set(Y))))
        ...
        if not self.exists(self.hashed_username) or self._should_update_model(clf, X, Y, conn):
        ...
        functions.set_team_training(conn, self.hashed_username, training=False)
    ...
    def _should_update_model(self, clf, x, y, conn):
        ...
        if mb_size < config.MAX_CLASSIFIER_SIZE_MB:
            ...
            if last_mean == 0 or float(last_mean - 0.01) < float(scores.mean()):
                return True
        return True
    ...
    def _get_training_data(self, conn):
        ...
        if X == [] or num_new < config.MIN_CLASSIFIER_TRAINING_IMAGES:
            return None, None, None, results
        return np.array(X)[:, 0], np.array(Y), np.array(weight), results
