...
def login_handler(request):
    ...
    if request.method == 'POST':
        context["form"] = form = forms.LoginForm(request.POST)
        if form.validate():
            ...
            if user:
                if user["confirmed_email"]:
                    ...
                    logout(request)
                    self.flash_error(
                        """You have not confirmed your email! <a href='/resend?email={}'>Resend confirmation?</a>""".format(
                            user["email"]
                        )
                    )
            else:
                self.flash_error(INVALID_MESSAGE)
