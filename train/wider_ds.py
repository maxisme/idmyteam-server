                ...
                if "/" in line:
                    ...
                    if tmp_path != current_path and os.path.isfile(tmp_path):
                        if len(file_bboxs) > 0:
                            if (
                                current_path
                                == "/var/www/f-server/data-sets/Faces/WIDER/WIDER_train/images/0--Parade/0_Parade_marchingband_1_849.jpg"
                            ):
                                print(file_bboxs)
                            self.paths.append(current_path)
                            file_bboxs = np.array(file_bboxs).astype(np.float32)
                            file_bboxs = file_bboxs[
                                :, [1, 0, 3, 2]
                            ]  # swap to bbox format used by chainer (y_min, x_min, y_max, x_max)
